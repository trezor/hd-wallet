...
export class BitcoreBlockchain {
    ...
    constructor(endpoints: Array<string>, socketWorkerFactory: SocketWorkerFactory, zcash: boolean) {
    ...
    lookupTransactionsStream(
        ...
        return Stream.fromPromise(
            this.socket.promise.then(socket => {
                ...
                ).map((r) => r.items.map((item) => historyToMatch(item, this.zcash)));
            })
        );
    }
    ...
    lookupTransaction(hash: string): Promise<TransactionInfo> {
        return this.socket.promise.then(socket =>
            lookupDetailedTransaction(socket, hash)
                .then((info) => detailedToInfo(info, false, this.zcash))
        );
    }
    ...
    lookupTransactionAsMatch(hash: string): Promise<TransactionMatch> {
        return this.socket.promise.then(socket =>
            lookupDetailedTransaction(socket, hash)
                .then((info) => detailedToTransactionMatch(info, true, this.zcash))
        );
    }
...
function historyToMatch(r: BcHistory, zcash: boolean): TransactionMatch {
    return {
        info: detailedToInfo(r.tx, true, zcash),
        addresses: Object.keys(r.addresses),
    };
}

function detailedToInfo(r: BcDetailedTransaction, neuter: boolean, zcash: boolean): TransactionInfo {
    return new TransactionInfo(
        bitcoreToTransaction(r, neuter, zcash),
        r.hash,
...
function detailedToTransactionMatch(r: BcDetailedTransaction, neuter: boolean, zcash: boolean): TransactionMatch {
    ...
    return {
        info: detailedToInfo(r, neuter, zcash),
        addresses,
    };
}
...
function bitcoreToTransaction(r: BcDetailedTransaction, neutered: boolean, zcash: boolean): Transaction {
    const tx = Transaction.fromHex(r.hex, zcash);
