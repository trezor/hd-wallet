...
export const initPromise: Promise<{accountInfo: ?AccountInfo, network: BitcoinJsNetwork, gap: number}> = initDfd.promise;

messageEmitter.attach((message, detach) => {
    if (message.type === 'init') {
        detach();
        initDfd.resolve({accountInfo: message.state, network: message.network, gap: message.gap});
    }
});
