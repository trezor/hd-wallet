...
channel.initPromise.then(({accountInfo, network, gap}) => {
...
function discoverAccount(
    ...
    return Promise.all([
        new GetChainTransactions(0, range, lastUsedAddresses[0], channel.chunkTransactions, transactions, mainAddresses, recvNetwork, recvGap).discover(),
        new GetChainTransactions(1, range, lastUsedAddresses[1], channel.chunkTransactions, [], changeAddresses, recvNetwork, recvGap).discover(),
